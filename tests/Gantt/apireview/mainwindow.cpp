    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->ganttView->leftView(), SIGNAL(customContextMenuRequested(const QPoint &)), this, SLOT(showContextMenu(const QPoint &)));
    connect(ui->ganttView->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
            this,
            SLOT(enableActions(const QItemSelection &)));

    connect(ui->ganttView->graphicsView(), SIGNAL(clicked(const QModelIndex &)), this, SLOT(slotClicked(const QModelIndex &)));
    connect(ui->ganttView->graphicsView(), SIGNAL(qrealClicked(const QModelIndex &)), this, SLOT(slotDoubleClicked(const QModelIndex &)));
}
...
void MainWindow::addDemoEntry()
{
    ...
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << codeFreeze << new MyStandardItem(KDGantt::TypeEvent) << new MyStandardItem(now, KDGantt::StartTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << packaging << new MyStandardItem(KDGantt::TypeTask) << new MyStandardItem(now.addDays(5), KDGantt::StartTimeRole)
                               << new MyStandardItem(now.addDays(10), KDGantt::EndTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>() << upload << new MyStandardItem(KDGantt::TypeTask)
    ...
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << testing << new MyStandardItem(KDGantt::TypeTask) << new MyStandardItem(now.addSecs(3 * 60 * 60), KDGantt::StartTimeRole)
                               << new MyStandardItem(now.addDays(5), KDGantt::EndTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>() << updateDocumentation << new MyStandardItem(KDGantt::TypeTask)
                                                        << new MyStandardItem(now.addSecs(3 * 60 * 60), KDGantt::StartTimeRole)
                                                        << new MyStandardItem(now.addDays(3), KDGantt::EndTimeRole));

    model->appendRow(QList<QStandardItem *>() << softwareRelease << new MyStandardItem(KDGantt::TypeSummary));
...
{
    statusBar()->showMessage(tr("(%1,%2,_,%4) clicked").arg(index.row()).arg(index.column()).arg((quint64)index.model()));
}
void MainWindow::slotDoubleClicked(const QModelIndex &index)
{
    statusBar()->showMessage(tr("(%1,%2,_,%4) qreal clicked").arg(index.row()).arg(index.column()).arg((quint64)index.model()));
}
