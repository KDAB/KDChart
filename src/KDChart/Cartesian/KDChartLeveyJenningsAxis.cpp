...
void LeveyJenningsAxis::paintAsOrdinate(PaintContext *context)
{
    ...
    TextLayoutItem labelItem(tr("mean"), labelTA, referenceArea, KDChartEnums::MeasureOrientationMinimum, Qt::AlignLeft);
...
void LeveyJenningsAxis::paintAsAbscissa(PaintContext *context)
{
    ...
    TextLayoutItem labelItem(range.first.date().toString(dateFormat()), labelTA, referenceArea, KDChartEnums::MeasureOrientationMinimum, Qt::AlignLeft);
    QSize origSize = labelItem.sizeHint();
    if (range.first.secsTo(range.second) < 86400)
        labelItem = TextLayoutItem(range.first.toString(dateFormat()), labelTA, referenceArea, KDChartEnums::MeasureOrientationMinimum, Qt::AlignLeft);
    ...
    TextLayoutItem labelItem2(range.second.date().toString(dateFormat()), labelTA, referenceArea, KDChartEnums::MeasureOrientationMinimum, Qt::AlignLeft);
    origSize = labelItem2.sizeHint();
    if (range.first.secsTo(range.second) < 86400)
        labelItem2 = TextLayoutItem(range.second.toString(dateFormat()), labelTA, referenceArea, KDChartEnums::MeasureOrientationMinimum, Qt::AlignLeft);
