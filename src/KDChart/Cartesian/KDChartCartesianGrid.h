...
public:
    ...
    [[nodiscard]] int minimalSteps() const;
    void setMinimalSteps(int minsteps);

    [[nodiscard]] int maximalSteps() const;
...
private:
    ...
    /**
     * Helper function called by calculateGrid() to calculate the grid of one
     * dimension.
     *
     * Classes derived from CartesianGrid can overwrite calculateGridXY() if
     * they need a special way of calculating the start or end or step width of
     * their grid lines.
     *
     ...
     */
    [[nodiscard]] virtual DataDimension
    calculateGridXY(const DataDimension &rawDataDimension, Qt::Orientation orientation, bool adjustLower, bool adjustUpper) const;

    /**
     * Helper function called by calculateGridXY().
     *
     * Classes derived from CartesianGrid can overwrite calculateStepWidth() if
     * they need a way of calculating the step width, based upon given start/end
     * values for their horizontal or vertical grid lines which is different
     * from the default implementation.
     *
     * \note The CartesianGrid class tries to keep the displayed range as close
     * to the raw data range as possible, so in most cases there should be no
     * reason to change the default implementation: Using
     * KDChart::GridAttributes::setGridGranularitySequence() should be
     * sufficient.
     *
     ...
     * \param adjustLower If true, the function adjusts the start value
     * so it matches the position of a grid line, if false the start value is
     * left as it is, in any case the value is adjusted for internal calculation
     * only. \param adjustUpper If true, the function adjusts the end value so
     * it matches the position of a grid line, if false the end value is left as
     * it is, in any case the value is adjusted for internal calculation only.
     *
