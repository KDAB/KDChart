...
public:
     ...
     */
    [[nodiscard]] const QPair<QPointF, QPointF> dataBoundaries() const;
     ...
     */
    [[nodiscard]] virtual bool usesExternalAttributesModel() const;
     ...
     * @see setAttributesModel
     */
    [[nodiscard]] virtual AttributesModel *attributesModel() const;
    ...
    [[nodiscard]] virtual QRegion visualRegion(const QModelIndex &index) const;
    /** \reimpl */
     ...
     * so neither the grid nor your axes' ranges will change, when you hide
     * data. For totally removing data from KD Chart's view you can use another
     * approach: e.g. you could define a proxy model on top of your data model,
     * and register the proxy model calling setModel() instead of registering
     * your real data model.
     *
     * @param index The datapoint to set the hidden status for. With a dataset
     * dimension of two, this is the index of the key of each key/value pair.
     ...
     */
    [[nodiscard]] bool isHidden(int dataset) const;
     ...
     */
    [[nodiscard]] bool isHidden(const QModelIndex &index) const;

    /**
     * Set the DataValueAttributes for the given index.
     * @param index The datapoint to set the attributes for. With a dataset
     * dimension of two, this is the index of the key of each key/value pair.
     ...
     */
    [[nodiscard]] DataValueAttributes dataValueAttributes() const;
     ...
     */
    [[nodiscard]] DataValueAttributes dataValueAttributes(int dataset) const;
     ...
     * are no datapoint specific settings.
     * @param index The datapoint to retrieve the attributes for. With a dataset
     * dimension of two, this is the index of the key of each key/value pair.
     * @return The DataValueAttributes for the given index.
     */
    [[nodiscard]] DataValueAttributes dataValueAttributes(const QModelIndex &index) const;
    ...
    /**
     * Retrieve the pen to be used for painting datapoints globally. This will
     * fall back automatically to the default settings, if there are no specific
     * settings.
     ...
     * @return The pen to use for painting.
     */
    [[nodiscard]] QPen pen(int dataset) const;
    /**
     * Retrieve the pen to be used, for painting the datapoint at the given
     * index in the model.
     * @param index The index of the datapoint in the model. With a dataset
     * dimension of two, this is the index of the key of each key/value pair.
     * @return The pen to use for painting.
     */
    [[nodiscard]] QPen pen(const QModelIndex &index) const;
    ...
    /**
     * Retrieve the brush to be used for painting datapoints globally. This will
     * fall back automatically to the default settings, if there are no specific
     * settings.
     ...
     * @return The brush to use for painting.
     */
    [[nodiscard]] QBrush brush(int dataset) const;
    /**
     ...
     * @return The brush to use for painting.
     */
    [[nodiscard]] QBrush brush(const QModelIndex &index) const;
     ...
     */
    [[nodiscard]] QString unitPrefix(int column, Qt::Orientation orientation, bool fallback = false) const;
    /**
     * Retrieves the axis unit prefix.
     * @param orientation The orientation of the axis.
     * @return The axis unit prefix.
     */
    [[nodiscard]] QString unitPrefix(Qt::Orientation orientation) const;
     ...
     */
    [[nodiscard]] QString unitSuffix(int column, Qt::Orientation orientation, bool fallback = false) const;
    /**
     * Retrieves the axis unit suffix.
     * @param orientation The orientation of the axis.
     * @return The axis unit suffix.
     */
    [[nodiscard]] QString unitSuffix(Qt::Orientation orientation) const;
     ...
     */
    [[nodiscard]] bool allowOverlappingDataValueTexts() const;
    ...
    /**
     * The set of item row labels currently displayed, for use in Abscissa axes,
     * etc.
     * @return The set of item row labels currently displayed.
     */
    [[nodiscard]] QStringList itemRowLabels() const;

    /**
     * The set of dataset labels currently displayed, for use in legends, etc.
     * @return The set of dataset labels currently displayed.
     */
    [[nodiscard]] QStringList datasetLabels() const;
     ...
     */
    [[nodiscard]] QList<QBrush> datasetBrushes() const;
     ...
     */
    [[nodiscard]] QList<QPen> datasetPens() const;
     ...
     * @return The current set of dataset brushes.
     */
    [[nodiscard]] QList<MarkerAttributes> datasetMarkers() const;

    /**
     * \deprecated
     *
     * \brief Deprecated method that turns the percent mode of this diagram on
     * or off.
     *
     * This method is deprecated. Use the setType() method of a supporting
     * diagram implementation instead, e.g. BarDiagram::setType().
     *
     * \see percentMode
     */
     ...
     * be drawn at the same X coordinate and stacked up so that the distance
     * from the last data point (or the zero line) to a data point P is always
     * the ratio of (Y-Value of P)/ (sum of all Y-Values in same column as P)
     * relative to the diagrams height (or width, if abscissa and ordinate are
     * swapped).
     *
     * Note that this property is not applicable to all diagram types.
     */
    [[nodiscard]] bool percentMode() const;
     ...
     * For each dimension and data series it will expect one column of values in
     * the model. If the dimension is 1, automatic values will be used for X.
     *
     * For example, a diagram with the default dimension of 1 will have one
     * column per data series (the Y values) and will use automatic values for X
     * (1, 2, 3, ... n).
...
public:
    ...
    spot. */
    [[nodiscard]] QModelIndexList indexesAt(const QPoint &point) const;
    [[nodiscard]] QModelIndexList indexesIn(const QRect &rect) const;

protected:
    [[nodiscard]] virtual bool checkInvariants(bool justReturnTheStatus = false) const;
...
protected:
     ...
     * which also is taking care for showing your cell-specific comments, if
     * any,
     */
    ...
    /**
     * Helper method, retrieving the data value (DisplayRole) for a given row
     * and column
     * @param row The row to query.
     * @param column The column to query.
     * @return The value of the display role at the given row and column as a
     * qreal.
     * @deprecated
     */
...
Q_SIGNALS:
    ...
    */
    ...
    /**
     * This signal is emitted when this diagram is being destroyed, but before
     * all the data, i.e. the attributes model, is invalidated.
     */
    void aboutToBeDestroyed();

    /** This signal is emitted when either the model or the AttributesModel is
     * replaced. */
    void modelsChanged();

    /** This signal is emitted just before the new attributes model is connected
       internally. It gives you a chance to connect to its signals first or
    ...
    /** This signal is emitted, when the hidden status of at least one data cell
     * was (un)set. */
    void dataHidden();
...
private:
    [[nodiscard]] QModelIndex conditionallyMapFromSource(const QModelIndex &sourceIndex) const;
};

using AbstractDiagramList = QList<AbstractDiagram *>;
using ConstAbstractDiagramList = QList<const AbstractDiagram *>;

/**
 * @brief Internally used class just adding a special constructor used by
 * AbstractDiagram
 */
