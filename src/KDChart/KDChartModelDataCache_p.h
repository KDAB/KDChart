...
public:
    ...
    {
        if (!index.isValid() || index.parent() != m_rootIndex || index.row() >= m_model->rowCount(m_rootIndex)
            || index.column() >= m_model->columnCount(m_rootIndex))
        ...
        if (index.row() >= m_data.count()) {
            ...
            const_cast<ModelDataCache<T, ROLE> *>(this)->rowsInserted(m_rootIndex, m_data.count(), m_model->rowCount(m_rootIndex) - 1);
        ...
        if (index.column() >= m_data.first().count()) {
            ...
            // apparently, data were added behind our back (w/o signals)
            const_cast<ModelDataCache<T, ROLE> *>(this)->columnsInserted(m_rootIndex, m_data.first().count(), m_model->columnCount(m_rootIndex) - 1);
...
protected:
    ...
    T fetchFromModel(int row, int column, int role) const
    {
        ...
        const T value = data.isNull() ? ModelDataCachePrivate::nan<T>() : (data.value<T>());
