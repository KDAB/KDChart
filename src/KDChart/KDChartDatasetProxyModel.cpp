...
void DatasetProxyModel::setDatasetRowDescriptionVector(const DatasetDescriptionVector &configuration)
{
...
QModelIndex DatasetProxyModel::index(int row, int column, const QModelIndex &parent) const
{
    return mapFromSource(sourceModel()->index(mapProxyRowToSource(row), mapProxyColumnToSource(column), parent));
}
...
void DatasetProxyModel::initializeDatasetDecriptors(const DatasetDescriptionVector &inConfiguration,
...
{
    if (sourceModel()) {
        disconnect(sourceModel(), SIGNAL(layoutChanged()), this, SLOT(resetDatasetDescriptions()));
    }
    QSortFilterProxyModel::setSourceModel(m);
    mRootIndex = QModelIndex();
    if (m) {
        connect(m, SIGNAL(layoutChanged()), this, SLOT(resetDatasetDescriptions()));
