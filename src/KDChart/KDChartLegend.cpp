...
void Legend::addDiagram(AbstractDiagram *newDiagram)
{
    if (newDiagram) {
        ...
        connect(observer, SIGNAL(diagramAboutToBeDestroyed(AbstractDiagram *)), SLOT(resetDiagram(AbstractDiagram *)));
        connect(observer, SIGNAL(diagramDataChanged(AbstractDiagram *)), SLOT(setNeedRebuild()));
        connect(observer, SIGNAL(diagramDataHidden(AbstractDiagram *)), SLOT(setNeedRebuild()));
        connect(observer, SIGNAL(diagramAttributesChanged(AbstractDiagram *)), SLOT(setNeedRebuild()));
        setNeedRebuild();
    }
}
...
void Legend::buildLegend()
{
    ...
    const KDChartEnums::MeasureOrientation measureOrientation =
        orientation() == Qt::Vertical ? KDChartEnums::MeasureOrientationMinimum : KDChartEnums::MeasureOrientationHorizontal;
    ...
    for (auto &hLayoutDataset : hLayoutDatasets) {
        ...
        if (!currentLine->isEmpty()) {
            ...
            if (currentLine->sizeHint().width() + separatorWidth + payloadWidth > allowedWidth) {
                // too wide, "line break"
#ifdef DEBUG_LEGEND_PAINT
                qDebug() << Q_FUNC_INFO << "break" << mainLayoutRow << currentLine->sizeHint().width()
