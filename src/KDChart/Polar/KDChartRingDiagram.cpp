    ...
    for (int iRow = 0; iRow < rCount; ++iRow) {
        ...
        for (int iColumn = 0; iColumn < colCount; ++iColumn) {
            ...
            const qreal cellValue = qAbs(model()
                                             ->data(model()->index(iRow, iColumn, rootIndex())) // checked
                                             .toReal(&bOK));
    ...
    if (angleLen) {
        ...
        if (angleLen == 360) {
            ...
            while (degree <= actualAngleLen) {
                const QPointF p = pointOnEllipse(drawPosition, dataset, slice, false, actualStartAngle + degree, totalRadialGap, totalRadialExplode);
                poly.append(p);
                degree += granularity;
                iPoint++;
            }
            if (!perfectMatch) {
                poly.append(pointOnEllipse(drawPosition, dataset, slice, false, actualStartAngle + actualAngleLen, totalRadialGap, totalRadialExplode));
                iPoint++;
            }
            ...
            while (degree >= 0) {
                poly.append(pointOnEllipse(drawPosition, dataset, slice, true, actualStartAngle + degree, totalRadialGap, totalRadialExplode));
            ...
            if (!perfectMatch) {
                poly.append(pointOnEllipse(drawPosition, dataset, slice, true, actualStartAngle, totalRadialGap, totalRadialExplode));
                iPoint++;
            }
 ...
 */
QPointF RingDiagram::pointOnEllipse(const QRectF &rect, int dataset, int slice, bool outer, qreal angle, qreal totalGapFactor, qreal totalExplodeFactor)
{
    qreal angleLen = d->angleLens[dataset][slice];
    qreal startAngle = d->startAngles[dataset][slice];
