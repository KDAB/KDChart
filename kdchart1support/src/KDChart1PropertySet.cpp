...
/**
  \file KDChart1PropertySet.cpp

  \brief Implementation of property settings of a single KDChart1 data cell.
  */

void KDChart1PropertySet::deepCopy( const KDChart1PropertySet* source )
{
...
KDChart1PropertySet* KDChart1PropertySet::clone() const
{
    KDChart1PropertySet* newSet = new KDChart1PropertySet();
    newSet->deepCopy( this );
    return newSet;
}

void KDChart1PropertySet::quickReset( const QString& name, int idParent )
{
    // set the name
    mName = name;
    // fill with idParent
...
void KDChart1PropertySet::fullReset( const QString& name, int idParent )
{
    quickReset( name, idParent );
    fillValueMembersWithDummyValues();
}

void KDChart1PropertySet::fillValueMembersWithDummyValues()
{
...
QDomElement KDChart1PropertySet::saveXML(QDomDocument& doc) const
{
    ...
    markerStElem.setAttribute("Style",
            KDChart1Params::lineMarkerStyleToString( (KDChart1Params::LineMarkerStyle)mMarkerStyle));
    // extra lines:
    ...
    specMarkerStElem.setAttribute("Style",
            KDChart1Params::lineMarkerStyleToString(    (KDChart1Params::LineMarkerStyle)mExtraMarkersStyle));
    return propertySetElement;
    */
}

bool KDChart1PropertySet::loadXML( const QDomElement& element, KDChart1PropertySet& set )
{
    Q_UNUSED(element);
    Q_UNUSED(set);
            ...
            while( !node.isNull() ) {
                ...
                if( !element.isNull() ) { // was really an element
                    ...
                    // normal line properties:
                        ...
                        if( element.hasAttribute( "Style" ) )
                            set.mMarkerStyle
                                = KDChart1Params::stringToLineMarkerStyle( element.attribute( "Style" ) );
                    } else
                        // extra lines:
                        ...
                        qDebug( "Unknown subelement of KDChart1PropertySet found: %s", tagName.latin1() );
                    }
                }
