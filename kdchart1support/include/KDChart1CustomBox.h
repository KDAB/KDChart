 ...
 **
 **********************************************************************/
#ifndef __KDCHART1CUSTOMBOX_H__
#define __KDCHART1CUSTOMBOX_H__
...
#define KDCHART1_AlignAuto 0


/** \file KDChart1CustomBox.h
...
/**
  ...
  */
class KDCHART1_EXPORT KDChart1CustomBox :public QObject
{
    Q_OBJECT
    friend class KDChart1Painter;
...
public:
    ...
    KDChart1CustomBox() :
        ...
        _color( Qt::black ),
        _paper( Qt::NoBrush ),
        _anchorArea(  KDChart1Enums::AreaInnermost ),
        _anchorPos(   KDChart1Enums::PosTopLeft ),
        _anchorAlign( Qt::AlignTop + Qt::AlignLeft ),
        _dataRow( 0 ),
        _dataCol( 0 ),
        _data3rd( 0 ),
        _deltaAlign( KDCHART1_AlignAuto ),
        _deltaScaleGlobal( true ),
        _anchorBeingCalculated( false )
    {
        const KDChart1TextPiece piece( 0, "", QFont( "helvetica", 8, QFont::Normal, false ) );
    ...
    /**
      ...
      */
    KDChart1CustomBox( const KDChart1TextPiece & content,
                      int fontSize,
    ...
    /**
      ...
      \param dataRow The row number of the KDChart1 data region that is to be used
      as anchor area. This parameter is ignored if \c area is not \c AreaChartDataRegion.
      \param dataCol The column number of the KDChart1 data region that is to be used
      as anchor area. This parameter is ignored if \c area is not \c AreaChartDataRegion.
      \param data3rd The third dimensions number of the KDChart1 data region that is to be used
      as anchor area. This parameter is ignored if \c area is not \c AreaChartDataRegion
      ...
      Leave this parameter to its default value KDCHART1_AlignAuto to have the delta values
      ...
      center delta alignment will cause KDChart1 to ignore the respective delta
    ...
    KDChart1CustomBox( const KDChart1TextPiece & content,
                      ...
                      uint area                           = KDChart1Enums::AreaInnermost,
                      KDChart1Enums::PositionFlag position = KDChart1Enums::PosTopLeft,
                      uint align                          = Qt::AlignTop + Qt::AlignLeft,
                      uint dataRow = 0,
                      uint dataCol = 0,
                      uint data3rd = 0,
                      uint deltaAlign = KDCHART1_AlignAuto,
    ...
    /**
      ...
      font size.
      */
    KDChart1CustomBox( int rotation,
                      const KDChart1TextPiece & content,
                      int fontSize,
                      bool fontScaleGlobal,
                      int deltaX,
                      int deltaY,
    ...
    /**
      ...
      object and fills a KDChart1CustomBox object with the data.
...
public slots: // PENDING(blackie) merge slots sections.
    ...
    /**
      ...
      (and dataRow(), dataCol(), data3rd() when dealing with KDChart1 data regions, resp.)
    ...
    /**
      ...
      */
    void setContent( const KDChart1TextPiece & content )
    {
        _content.deepCopy( &content );
    }
    ...
    /**
      ...
      */
    void setAnchorPosition( KDChart1Enums::PositionFlag position )
    {
        // Note if you change the parameters here, then you must also change them in wrappers/KDChart1CustomBoxWrapper.h
        _anchorPos = position;
    }
    ...
    /**
      Specifies the row number of the KDChart1 data region that is to be used
    ...
    /**
      Specifies the column number of the KDChart1 data region that is to be used
      as anchor area. This value is ignored if anchorArea is not \c AreaChartDataRegion.
      */
    ...
    /**
      Specifies the third dimensions number of the KDChart1 data region that is to be used
      as anchor area. This value is ignored if anchorArea is not \c AreaChartDataRegion
      or if there is no 3-dimensional data structure.
      */
    ...
    /**
      ...
      Set this to KDHART_KDCHART1_AlignAuto to have the delta values
      used according to the box's main \c align settings, otherwise specify your own
      alignment settings: e.g. right means there will be a gap between the right side of
      the box and its anchor point - if the main \c align parameter is set to right too
      the anchor point will to be outside of the box / if \c align is set to left
    ...
    /**
      ...
      */
    const KDChart1TextPiece & content() const
    {
        return _content;
    }
    ...
    /**
      ...
      to which the box is to be aligned.
      */
    KDChart1Enums::PositionFlag anchorPosition() const
    {
        return _anchorPos;
    }
    ...
    /**
      Returns the row number of the KDChart1 data region that is to be used
    ...
    /**
      Returns the column number of the KDChart1 data region that is to be used
      as anchor area. You should use this if anchorArea is \c AreaChartDataRegion
      to find out the data region which the box is to be aligned to.
      */
    uint dataCol() const
    ...
    /**
      Returns the third dimensions number of the KDChart1 data region that is to be used
      as anchor area. You should use this if anchorArea is \c AreaChartDataRegion
...
public:
    ...
    /**
      ...
      \sa clone
      */
    void deepCopy( const KDChart1CustomBox* source );
    ...
    /**
      ...
      \sa deepCopy
      */
    KDChart1CustomBox* clone() const;

private:
    KDChart1CustomBox( const KDChart1CustomBox& ) : QObject(0) {}
...
protected:
    ...
    {
        KDChart1CustomBox* that = const_cast<KDChart1CustomBox*>(this);
...
private:
    ...
    uint                       _anchorArea;
    KDChart1Enums::PositionFlag _anchorPos;
    uint                       _anchorAlign;
    uint _dataRow;
    uint _dataCol;
    uint _data3rd;
    ...
    // after loading a KDChart1CustomBox from a file.
    bool _anchorBeingCalculated;
};


/**
  Writes the KDChart1CustomBox object p as an XML document to the text stream s.

  \param s the text stream to write to
  \param p the KDChart1CustomBox object to write
  \return the text stream after the write operation
  */
//QTextStream& operator<<( QTextStream& s, const KDChart1CustomBox& p );


/**
  Reads the an XML document from the text stream s into the
  KDChart1CustomBox object p

  \param s the text stream to read from
  \param p the KDChart1CustomBox object to read into
  \return the text stream after the read operation
  */
//QTextStream& operator>>( QTextStream& s, KDChart1CustomBox& p );
