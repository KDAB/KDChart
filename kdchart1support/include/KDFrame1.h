 ...
 **
 **********************************************************************/
#ifndef __KDFRAME1_H__
#define __KDFRAME1_H__
...
/** \file KDFrame1.h
  \brief Header of the KDFrame1 class providing highly configurable rectangular frame drawing.
  ...
  See KDChart1Params::setSimpleFrame() to learn how to use them.
  \else
...
/**
  \brief The main class of KDFrame1.

  KDFrame1 is made for highly configurable rectangular frame drawing.
  ...
  \endif
  */
class KDCHART1_EXPORT KDFrame1 : public QObject
{
    Q_OBJECT
...
public slots:
    ...
    */
    void addProfileSection( ProfileName      name,
            int              wid,
            QPen             pen,
            KDFrame1ProfileSection::Direction dir  = KDFrame1ProfileSection::DirPlain, // PENDING(blackie) possible enum problem
            KDFrame1ProfileSection::Curvature curv = KDFrame1ProfileSection::CvtPlain );

    /**
    \ifnot v200
    \deprecated Feature scheduled for future release, at present not implemented.
    \else
    Specify one of the frames profiles by copying another KDFrame1Profile.
    ...
    KDFrame1 myFrame( 50,20, 250,120 );

    // select a very simple rectangular frame with normal corners, black border, white background
    myFrame.setSimpleFrame( KDFrame1::FrameFlat, 1, 0, QPen( Qt::Black ), QBrush( Qt::White ) );

    // make the top profile look more nice
    myFrame.clearProfile(      KDFrame1::ProfileTop );
    myFrame.addProfileSection( KDFrame1::ProfileTop, 2, QPen( Qt::Black ),
    KDFrame1ProfileSection::DirPlain,
    KDFrame1ProfileSection::CvtPlain );
    myFrame.addProfileSection( KDFrame1::ProfileTop, 5, QPen( Qt::NoPen ),
    KDFrame1ProfileSection::DirPlain,
    KDFrame1ProfileSection::CvtPlain );
    myFrame.addProfileSection( KDFrame1::ProfileTop, 1, QPen( Qt::Black ),
    KDFrame1ProfileSection::DirPlain,
    KDFrame1ProfileSection::CvtPlain );

    // copy the top profile settings into the bottom profile
    myFrame.setProfile( KDFrame1::ProfileBottom,
    myFrame.profile( KDFrame1::ProfileTop ) );
    \endverbatim
    ...
    \endif
    */
    void setProfile( ProfileName name, const KDFrame1Profile& profile );
    ...
    \endif
    */
    const KDFrame1Profile& profile( ProfileName name ) const;
...
// MOC_SKIP_BEGIN
        ...
        the appropriate methods of the KDFrame1 class.

        \sa setCorners, setCorner, cornerStyle, cornerWidth, cornerProfile, CornerName, CornerStyle
        \endif
        */
    class KDCHART1_EXPORT KDFrame1Corner
        {
            friend class KDFrame1;
            public:
            KDFrame1Corner( CornerStyle           style   = CornerNormal,
                    int                   width   = 0,
                    KDFrame1Profile* const profile = 0 )
                : _style( style ),
            _width( width )
            {
                if( profile )
            ...
            */
            ...
            corner and fills a KDFrame1Corner object with the data.
            ...
            */
            ...
            const KDFrame1Profile& profile() const
            {
                return _profile;
            }
            private:
...
public slots:
        ...
        */
        ...
        \endif
        */
        const KDFrame1Profile& cornerProfile( CornerName name ) const
        {
            switch( name ) {
                case CornerUNKNOWN:
...
public:
        ...
        \sa paint
        */
        enum KDFrame1PaintSteps { PaintBackground, PaintEdges, PaintCorners, PaintBorder, PaintAll };
...
public slots:
        ...
        \param steps The part of the frame to be drawn, use KDFrame1::PaintAll to draw the entire frame and the background,
        use KDFrame1::PaintBackground to draw only the background, use KDFrame1::PaintEdges to draw just the edges,
        use KDFrame1::PaintCorners to draw only the corners.
        \param innerRect The area inside the frame. Use this parameter to temporarily override the \c innerRect set by
        the constructor of KDFrame1 or by setInnerRect(). This approach can be usefull if you want to draw several
        frames that differ only regarding to their position and size but share the same edges/corners/background settings.
        In this case you could decide to instantiate only one KDFrame1 set up the desired settings and just call
        its paint() methode several time - giving it the appropriate innerRect for each frame. This would result in
        less memory usage since you could use that single KDFrame1 object as kind of a shared ressource.
        */
        virtual void paint( QPainter* painter,
                KDFrame1PaintSteps steps = PaintAll,
...
public:
    ...
    */
    KDFrame1( QRect          innerRect      = QRect(0,0,0,0),
    ...
    {
        _profileSectionsAutoDelete = true;
        _innerRect = innerRect;
...
*/
private:
    KDFrame1( const KDFrame1& ) : QObject(0) { _profileSectionsAutoDelete=true; }
    KDFrame1& operator=( const KDFrame1& ){return *this;}
...
public:
    ...
    */
    static void deepCopy( KDFrame1& D, const KDFrame1& R )
    {
        ...
        D.setProfileSectionsAutoDelete( true );
        R.setProfileSectionsAutoDelete( false );
    }

    friend QTextStream& operator<<( QTextStream& s, const KDFrame1& p );
    friend QTextStream& operator>>( QTextStream& s, KDFrame1& p );
    ...
    */
    ...
    /**
    Reads data from a DOM element node that represents a frame
    object and fills a KDFrame1 object with the data.
    ...
    */
    ...
    /**
    Reads data from a DOM element node that represents a frame
    profile and fills a KDFrame1Profile object with the data.
...
private:
    ...
    {
        _profileSectionsAutoDelete = on;
    }

    void deleteProfileSections();
...
/**
  ...
  \param p the KDFrame1 object to read into
  \return the text stream after the read operation
  */
QTextStream& operator>>( QTextStream& s, KDFrame1& p );
