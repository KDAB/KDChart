 ...
 **
 **********************************************************************/
#ifndef __KDCHART1PARAMS_H__
#define __KDCHART1PARAMS_H__
...
/** \file KDChart1Params.h
    \brief Header for all common chart parameters.

    This file is used to access all chart parameters except of the
    axis settings which you will find in \c KDChart1AxisParams.h

    \see KDChart1AxisParams.h
*/


typedef QHash<int, KDChart1PropertySet* > KDChart1PropertySetList;
//typedef QIntDict<KDChart1PropertySet> KDChart1PropertySetList;

#define KDCHART1_ALL_AXES UINT_MAX-1
...
//       in KDChart1AxisParams::KDChart1AxisParams() to initialize the
//       axis' _axisIsoRefAxis member.
//       OK, this is a dirty hack but is avoids circular dependency
//       between KDChart1Params and KDChart1AxisParams
#define KDCHART1_NO_AXIS UINT_MAX
#define KDCHART1_ALL_DATASETS UINT_MAX - 1

#define KDCHART1_NO_DATASET UINT_MAX
#define KDCHART1_UNKNOWN_CHART UINT_MAX - 2
#define KDCHART1_ALL_CHARTS UINT_MAX - 1
#define KDCHART1_NO_CHART UINT_MAX
#define KDCHART1_GLOBAL_LINE_STYLE UINT_MAX
#define KDCHART1_AUTO_SIZE INT_MAX
#define KDCHART1_DATA_VALUE_AUTO_DIGITS INT_MAX
#define KDCHART1_SAGITTAL_ROTATION INT_MAX
#define KDCHART1_TANGENTIAL_ROTATION INT_MAX - 1
#define KDCHART1_PROPSET_NORMAL_DATA 0
#define KDCHART1_PROPSET_TRANSPARENT_DATA 1
#define KDCHART1_PROPSET_HORI_LINE  2
#define KDCHART1_PROPSET_VERT_LINE  3

// #define KDCHART1_DATA_VALUE_AUTO_COLOR KDChart1AutoColor::instance()->color()

// PENDING(blackie) Can we delete this now?
/* we must keep this wrongly spelled value for backward compatibility reasons */
#define KDCHART1_SAGGITAL_ROTATION INT_MAX

/**
  Our charts may have up to 4 ordinate axes:
  2 left ones and 2 right ones
  */
#define KDCHART1_CNT_ORDINATES 4

#define KDCHART1_MAX_POLAR_DELIMS_AND_LABELS_POS 8
#define KDCHART1_MAX_AXES 12


#define KDCHART1_DATA_VALUE_AUTO_COLOR (KDChart1AutoColor::instance()->color())

class KDCHART1_EXPORT KDChart1AutoColor {
public:
    static const KDChart1AutoColor* instance();
...
private:
    KDChart1AutoColor();
    KDChart1AutoColor( KDChart1AutoColor const& );
    ~KDChart1AutoColor();
    QColor mColor;
    static KDChart1AutoColor *mInstance;
};
...
public slots: // PENDING(blackie) merge public slots into one section.
    ...
    int registerProperties( KDChart1PropertySet& rSet );
    void setProperties( int id, KDChart1PropertySet& rSet );
    bool removeProperties( int id );
    bool properties( int id, KDChart1PropertySet& rSet ) const;
    KDChart1PropertySet* properties( int id );
    bool calculateProperties( int startId, KDChart1PropertySet& rSet ) const;

public:

// MOC_SKIP_BEGIN
    class KDCHART1_EXPORT KDChart1FrameSettings
    {
        // Q_OBJECT
    public:
        KDChart1FrameSettings();
        KDChart1FrameSettings( uint           dataRow,
                              uint           dataCol,
                              uint           data3rd,
        ...
        const KDFrame1* framePtr() const
            {
                return _frame;
            }


        const KDFrame1& frame() const
            {
                return *_frame;
            }
        ...
        // public:
        virtual ~KDChart1FrameSettings();


        static void createFrameSettingsNode( QDomDocument& document,
                                             QDomNode& parent,
                                             const QString& elementName,
                                             const KDChart1FrameSettings* settings,
                                             uint areaId );
        ...
        static void deepCopy( KDChart1FrameSettings& D, const KDChart1FrameSettings& R ) {
            ...
            delete D._frame;
            if( R._frame ){
                D._frame = new KDFrame1();
                KDFrame1::deepCopy(*D._frame, *R._frame);
            }else{
                D._frame = 0;
            }
    ...
    private:
        // KDChart1FrameSettings( const KDChart1FrameSettings& other ) :QObject( 0 ) {
        KDChart1FrameSettings( const KDChart1FrameSettings& ) {}
        KDChart1FrameSettings& operator=( const KDChart1FrameSettings& ){return *this;}
...
public slots:
    ...
    const KDChart1FrameSettings* frameSettings( uint area,
                                               bool& bFound,
                                               int* pIterIdx=0 ) const;

    const KDChart1FrameSettings* nextFrameSettings( bool& bFound,
                                                   int* pIterIdx ) const;


public:
    typedef QHash<int, KDChart1CustomBox* > CustomBoxDict;
    //typedef QIntDict <KDChart1CustomBox > CustomBoxDict;

public slots:
    uint insertCustomBox( const KDChart1CustomBox & box );
    ...
    KDChart1CustomBox* customBoxRef( uint box );
    const KDChart1CustomBox* customBox( uint box ) const;
...
public slots:
    void setChartSourceMode( SourceMode mode,
                             uint dataset,
                             uint dataset2 = KDCHART1_NO_DATASET,
                             uint chart = 0 );
    ...
    void setPrintDataValues( bool active,
                             uint chart = KDCHART1_ALL_CHARTS,
                             int divPow10 = 0,
                             int digitsBehindComma = KDCHART1_DATA_VALUE_AUTO_DIGITS,
                             QFont* font   = 0,
                             uint size     = UINT_MAX, //  <-- makes us use the *default* font params
                             //                                by IGNORING settings of
                             //                                the following parameters!
                             const QColor* color = KDCHART1_DATA_VALUE_AUTO_COLOR,
                             KDChart1Enums::PositionFlag negativePosition = KDChart1Enums::PosCenter,
                             uint negativeAlign    = Qt::AlignCenter,
                             int  negativeDeltaX   =    0,
                             int  negativeDeltaY   =    0,
                             int  negativeRotation =    0,
                             KDChart1Enums::PositionFlag positivePosition = KDChart1Enums::PosCenter,
                             uint positiveAlign    = Qt::AlignCenter,
                             int  positiveDeltaX   =    0,
                             int  positiveDeltaY   =    0,
                             int  positiveRotation =    0,
                             KDChart1Enums::TextLayoutPolicy policy = KDChart1Enums::LayoutPolicyRotate );
    ...
    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    void setDataValuesPlacing( KDChart1Enums::PositionFlag position,
                               uint align,
                               int  deltaX,
                               int  deltaY,
                               int  rotation,
    ...
    void setDataValuesColors( const QColor* color = KDCHART1_DATA_VALUE_AUTO_COLOR,
                              const QBrush& background = Qt::NoBrush,
                              uint chart = KDCHART1_ALL_CHARTS );

    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    void setDataValuesPolicy( KDChart1Enums::TextLayoutPolicy policy = KDChart1Enums::LayoutPolicyRotate,
                              uint chart = KDCHART1_ALL_CHARTS );

    void setDataValuesShowInfinite( bool dataValuesShowInfinite = true,
                                    uint chart = KDCHART1_ALL_CHARTS );

    void setPrintDataValuesWithDefaultFontParams( uint chart = KDCHART1_ALL_CHARTS,
    ...
    KDChart1Enums::PositionFlag dataValuesAnchorPosition( uint chart,
                                                         bool negative ) const;
    ...
    KDChart1Enums::TextLayoutPolicy dataValuesLayoutPolicy( uint chart ) const
        {
    ...
    void setPrintDataValuesColor( uint chart = KDCHART1_ALL_CHARTS,
                                  const QColor* color = KDCHART1_DATA_VALUE_AUTO_COLOR );
...
public slots:
    ...
    void setLineStyle( Qt::PenStyle style, uint dataset=KDCHART1_GLOBAL_LINE_STYLE );

    Qt::PenStyle lineStyle( uint dataset=KDCHART1_GLOBAL_LINE_STYLE ) const;
...
public slots:
    ...
    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    void setPolarDelimsAndLabelsAtPos( KDChart1Enums::PositionFlag pos,
                                       bool showDelimiters,
                                       bool showLabels );

    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    bool polarDelimAtPos( KDChart1Enums::PositionFlag pos ) const;

    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    bool polarLabelsAtPos( KDChart1Enums::PositionFlag pos ) const;
    ...
    void setExplodeValues( QList<int> explodeList ) {
...
public slots:
    ...
    // Note if you change the parameters here, then you must also change them in wrappers/KDChart1ParametersWrapper.h
    void setAxisType( uint n, const KDChart1AxisParams::AxisType axisType )
        {
    ...
    bool axisVisible( uint n ) const
        {
            return n < KDCHART1_MAX_AXES ? _axisSettings[ n ].params.axisVisible()
                : false;
        }
...
public slots:
    ...
    void setAxisLabelTextParams( uint n,
                                 ...
                                 QVariant axisValueStart = KDCHART1_AXIS_LABELS_AUTO_LIMIT,
                                 QVariant axisValueEnd   = KDCHART1_AXIS_LABELS_AUTO_LIMIT,
                                 double axisValueDelta      = KDCHART1_AXIS_LABELS_AUTO_DELTA,
                                 int axisDigitsBehindComma  = KDCHART1_AXIS_LABELS_AUTO_DIGITS,
                                 ...
                                 int axisValueLeaveOut      = KDCHART1_AXIS_LABELS_AUTO_LEAVEOUT,
                                 KDChart1AxisParams::ValueScale axisValueDeltaScale = KDChart1AxisParams::ValueScaleNumerical )
        {
    ...
    const KDChart1AxisParams& axisParams( uint n ) const
        {
            return n < KDCHART1_MAX_AXES ? _axisSettings[ n ].params
                : _axisSettings[ KDCHART1_MAX_AXES ].params;
        }

    // some convenience functions refering to the first KDChart1CustomBox
...
public:
    ...
    /*
      ...
      algorithms in
      KDChart1Painter::setupGeometry() and
      KDChart1Painter::paintHeaderFooter() rely on this schema.
...
public:
    friend QTextStream& operator<<( QTextStream& s, const KDChart1Params& p );
    friend QTextStream& operator>>( QTextStream& s, KDChart1Params& p );
...
public slots:
    ...
    // do *not* call this function manually: it is used by KDChart1Painter::setupGeometry()
...
private:
    ...
    KDChart1PropertySet* tempPropSetA;
    KDChart1PropertySet* tempPropSetB;
    ...
    typedef QHash<QString, KDChart1FrameSettings* > AreaDict;
    AreaDict _areaDict;
    ...
    KDChart1FrameSettings _noFrameSettings;

    class ModeAndChart
    {
    public:
        ModeAndChart()
            : _mode( UnknownMode ), _chart( KDCHART1_NO_CHART )
            {}
    ...
    KDChart1PropertySetList _propertySetList;
    ...
    struct PrintDataValuesSettings {
        ...
        KDChart1Enums::PositionFlag _dataValuesAnchorNegativePosition;
        ...
        KDChart1Enums::PositionFlag _dataValuesAnchorPositivePosition;
        ...
        KDChart1Enums::TextLayoutPolicy _dataValuesLayoutPolicy;
    ...
    _polarDelimsAndLabelStruct _polarDelimsAndLabels[ 1 + KDCHART1_MAX_POLAR_DELIMS_AND_LABELS_POS ];
    ...
    struct KDCHART1_EXPORT AxisSettings
    {
        uint dataset;
        uint dataset2;
        uint chart;
        KDChart1AxisParams params;
        AxisSettings()
            {
                dataset = KDCHART1_NO_DATASET;
                dataset2 = KDCHART1_NO_DATASET;
                chart = KDCHART1_NO_CHART;
            }
    };

    // 13 == KDCHART1_MAX_AXES + 1
    ...
    // Must be exported because of the inline methods
    struct KDCHART1_EXPORT HdFtParams
    {
        QString _text;
        QColor _color;
        QFont _font;
