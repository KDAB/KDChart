...
private slots:
    ...
    void mapToCacheTest()
    {
        ...
        Match matches[] = {Match(CachePosition(0, 0), model.index(0, 0)),
    ...
    void invalidateTest()
    {
        ...
        QVERIFY2(compressor.isCached(position), "datapoint should be valid after retrieveModelData( position )");
        QModelIndex index(model.index(0, 0));
        compressor.slotModelDataChanged(index, index);
        QVERIFY2(!compressor.isCached(position), "datapoint should be not valid after call to dataChanged slot");
    ...
    void datasetDimensionTest()
    {
        ...
        QVERIFY2(compressor.modelDataColumns() == ColumnCount / 2, "datasetDimension == 2 should cut the column count in half");
        compressor.setDatasetDimension(oldDimension);
        QVERIFY2(compressor.modelDataColumns() == ColumnCount, "datasetDimension == 1 should restore the old column count");
    }
