...
void CoordinatePlane::paintEvent( QPaintEvent* e )
{
    ...
    { // the grid lines:
        ...
        {   // draw 5 horizontal and vertical grid lines
            lines.append( QLineF( transformation.translate ( QPointF( m_topRight.x() - i, m_topRight.y() ) ),
                                  transformation.translate ( QPointF( m_topRight.x() - i, 0.0 ) ) ) );
        ...
        {
            lines.append( QLineF( transformation.translate ( QPointF( 0.0, m_topRight.y() - i ) ),
                                  transformation.translate ( QPointF( m_topRight.x(), m_topRight.y() - i ) ) ) );
    ...
    { // the zero lines:
        ...
        painter.drawLine( transformation.translate ( QPointF( 0.0, 0.0 ) ),
                          transformation.translate ( QPointF ( m_topRight.x(), 0.0 ) ) );
        painter.drawLine( transformation.translate ( QPointF( 0.0, m_topRight.y() ) ),
                          transformation.translate ( QPointF ( 0.0, 0.0 ) ) );
    }

    for ( int i = 1; i < ( int ) widthF() - 1.1; ++i )
    {
        drawBar ( painter, i, i );
    }
}
